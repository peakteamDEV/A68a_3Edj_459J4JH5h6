...
if os.path.exists(CATALOG_CACHE):
    with open(CATALOG_CACHE, "r", encoding="utf-8") as f:
        cached = json.load(f)
    if time.time() - cached.get("fetched_at", 0) < CATALOG_MAX_AGE:
        catalog = {int(k): v for k, v in cached["items"].items()}
        print(f"using cached catalog ({len(catalog)} items)")
...
if catalog is None:
    ...
    with open(CATALOG_CACHE, "w", encoding="utf-8") as f:
        json.dump({"fetched_at": time.time(), "items": catalog}, f)
    print(f"catalog fetched and cached: {len(catalog)} items")

def price_items(item_ids):
    # id is attached HERE, from the id we already have in hand -- never
    # trusted from inside the cached catalog value itself, so a schema
    # change to the cache can't silently break this again
    out = []
    for iid in item_ids:
        if iid in catalog:
            d = dict(catalog[iid])
...
for ad_id, ts, player_id, username, offer, request in ads:
    ...
    request_items = price_items(request_item_ids)
...
print(f"resolved {len(thumbs)}/{len(need_ids)} thumbnails")
...
print(f"archive now holds {len(archive)} ads total ({new_count} new this run, "
      f"{sum(1 for a in archive.values() if a['active'])} currently active)")
...
function itemSlot(i) {
  return i.thumb
    ? `<div class="slot"><img src="${i.thumb}" title="${i.name}"></div>`
    : `<div class="slot"></div>`;
}
function tagSlot(t) {
  return `<div class="slot"><img src="${t.icon}" title="${t.name}">
    <div class="tagname">${t.name}</div></div>`;
}
...
document.getElementById('cards').innerHTML = DATA.ads.map(a => {
  ...
  const statusBadge = a.is_new ? '<span class="badge">NEW</span>'
    : (!a.active ? '<span class="badge gone">GONE</span>' : '');
  ...
  return `<div class="card ${a.is_new ? 'new' : ''} ${!a.active ? 'gone' : ''}">
    ...
    <div class="cols">
      ...
      <div class="col">
        ...
        <div class="stats">
          ${a.request_robux > 0 ? `<div class="rbx">Asking ${a.request_robux.toLocaleString()} Robux</div>` : '<div class="rbx">Robux accepted</div>'}
        </div>
      </div>
    </div>
