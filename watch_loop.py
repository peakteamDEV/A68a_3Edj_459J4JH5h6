...
MIN_VALUE = 5000
MAX_VALUE = 50000
POLL_SECONDS = 30  # Rolimons' own docs say "avoid rapid successive requests" -- 30s is brisk but not hammering them
...
def load_catalog():
    if os.path.exists(CATALOG_CACHE):
        ...
        if time.time() - cached.get("fetched_at", 0) < CATALOG_MAX_AGE:
            return {int(k): v for k, v in cached["items"].items()}
...
if __name__ == "__main__":
    ...
    print(f"starting persistent loop, polling every {POLL_SECONDS}s -- Ctrl+C to stop")
    while True:
        try:
            run_once(catalog)
        except Exception:
